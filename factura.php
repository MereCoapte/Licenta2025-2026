...
// Verificare acces:
// - Admin: vede orice factura
// - User logat: doar comenzile lui
// - Guest: nu poate vedea factura (nu are sesiune)
if (!$isAdmin) {
...
// Numele si emailul clientului — user sau guest
$isGuestOrder  = is_null($order['user_id']);
$clientName    = $isGuestOrder ? ($order['guest_name']  ?? 'Guest') : ($order['client_name']  ?? 'N/A');
$clientEmail   = $isGuestOrder ? ($order['guest_email'] ?? '—')     : ($order['client_email'] ?? '—');
$clientPhone   = $isGuestOrder ? ($order['guest_phone'] ?? '—')     : '—';

// Produsele
$stmt = $pdo->prepare("
    SELECT oi.quantity, oi.price, (oi.quantity * oi.price) AS subtotal,
...
<head>
    ...
    <style>
        ...
        .action-bar { background:#1a1a2e; padding:14px 32px; display:flex; justify-content:space-between; align-items:center; }
        .invoice-wrapper { max-width:820px; margin:32px auto; background:#fff; border-radius:12px; box-shadow:0 4px 24px rgba(0,0,0,0.10); overflow:hidden; }
        .invoice-header { background:#1a1a2e; color:#fff; padding:36px 40px 28px; }
        .invoice-header .brand-name { font-size:28px; font-weight:800; letter-spacing:-0.5px; }
        .invoice-header .brand-name span { color:#e84700; }
        .invoice-header .invoice-title { font-size:13px; color:#aaa; letter-spacing:3px; text-transform:uppercase; margin-top:4px; }
        .invoice-nr { text-align:right; }
        .invoice-nr .nr { font-size:22px; font-weight:700; color:#e84700; }
        .invoice-nr .data { font-size:13px; color:#aaa; margin-top:4px; }
        ...
        .party-box .party-label { font-size:11px; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:#e84700; margin-bottom:10px; }
        ...
        .invoice-table thead tr { background:#1a1a2e; color:#fff; }
        .invoice-table thead th { padding:12px 16px; font-size:11px; font-weight:600; letter-spacing:1px; text-transform:uppercase; }
        ...
        .totals-section { display:flex; justify-content:flex-end; padding:24px 40px; background:#f8f9fa; }
        .totals-box { min-width:280px; }
        .totals-row { display:flex; justify-content:space-between; padding:6px 0; font-size:14px; color:#555; }
        .totals-row.total-final { border-top:2px solid #1a1a2e; margin-top:8px; padding-top:12px; font-size:18px; font-weight:800; color:#1a1a2e; }
        ...
        .invoice-footer { padding:28px 40px; border-top:1px solid #f0f0f0; }
        .signature-section { display:grid; grid-template-columns:1fr 1fr; gap:40px; margin-bottom:28px; }
        .signature-box { text-align:center; }
        .signature-label { font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; color:#888; margin-bottom:48px; }
        .signature-line { border-top:1px solid #ccc; padding-top:8px; font-size:12px; color:#aaa; }
        .legal-text { font-size:11px; color:#bbb; text-align:center; line-height:1.8; }
        @media print {
            body { background:#fff !important; }
            .action-bar { display:none !important; }
            .invoice-wrapper { box-shadow:none !important; border-radius:0 !important; margin:0 !important; max-width:100% !important; }
        }
    </style>
</head>
<body>
...
<div class="invoice-wrapper">
    ...
    <div class="invoice-header">
        <div class="d-flex justify-content-between align-items-start">
            ...
            <div class="invoice-nr">
                ...
                <div class="data mt-1">
                    <?php
                    $statusColors = ['pending'=>'#ffc107','processing'=>'#0dcaf0','shipped'=>'#0d6efd','delivered'=>'#198754'];
                    $statusLabels = ['pending'=>'In asteptare','processing'=>'In procesare','shipped'=>'Expediat','delivered'=>'Livrat'];
        ...
        <div class="parties">
            ...
            <div class="party-box">
                <div class="party-label">
                    Client
                    <?php if ($isGuestOrder): ?>
                        <span style="background:#6c757d22;color:#6c757d;border:1px solid #6c757d44;
                                     padding:2px 8px;border-radius:10px;font-size:10px;margin-left:6px;">
                            Vizitator
                        </span>
                    <?php endif; ?>
                </div>
                <div class="party-name"><?= htmlspecialchars($clientName) ?></div>
                <div class="party-detail">
                    <?= htmlspecialchars($clientEmail) ?><br>
                    <?php if ($isGuestOrder && $clientPhone !== '—'): ?>
                        📞 <?= htmlspecialchars($clientPhone) ?><br>
                    <?php endif; ?>
    ...
    <div class="invoice-footer">
        <div class="signature-section">
            ...
            <div class="signature-box">
                <div class="signature-label">Client — <?= htmlspecialchars($clientName) ?></div>
